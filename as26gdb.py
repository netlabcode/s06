...
def serverOne():
	with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as sc1:
		...
		while x < 6:
			...
			cursor.execute(" INSERT INTO s26m1(dtime, cb_ctrl, cb_res, i_res, p_res, q_res, v_res) VALUES (%s,%s,%s,%s,%s,%s,%s)", inserted_values)
...
def serverTwo():
	with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as sc1:
		...
		while x < 6:
			...
			cursor.execute(" INSERT INTO s26m2(dtime, cb_ctrl, cb_res, i_res, p_res, q_res, v_res) VALUES (%s,%s,%s,%s,%s,%s,%s)", inserted_values)
...
def serverThree():
	with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as sc1:
		...
		while x < 6:
			...
			cursor.execute(" INSERT INTO s26m3(dtime, cb_ctrl, cb_res, f_res, i_res, p_res, q_res, v_res) VALUES (%s,%s,%s,%s,%s,%s,%s,%s)", inserted_values)
